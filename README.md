...
In the example, an agent is trained using Deep Deterministic Policy Gradient (DDPG) over a series of episodes. It begins with no knowledge of its environment, and first learns only by taking actions at random. The agent's experiences are catalogued and it gradually begins making decisions to maximize its reward instead of choosing random actions. The environment is solved once the agent achieves an average score of +0.5 over 100 consecutive episodes.
